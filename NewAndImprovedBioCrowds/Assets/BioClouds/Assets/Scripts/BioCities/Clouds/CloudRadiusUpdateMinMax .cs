...
namespace BioCities {
    ...
    public class CloudRadiusUpdateMinMax : JobComponentSystem
    {
        ...
        struct CorrectRadiusJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                float beta = (math.pow(delta,2f) / math.pow(cData.PreferredDensity,2f)); 
                //float beta = (math.sqrt(delta) / math.sqrt(cData.PreferredDensity));

                cData.Radius = math.max(math.min((cData.Radius * math.sqrt(beta) + cData.Radius) /2f , MaxRadius), MinRadius);
